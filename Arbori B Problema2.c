// Sa se redacteze un program, format dintr-un singur fisier .c. Programul implementeaza urmatoarele operatii (in ordinea data) pentru prelucrarea unui arbore B de ordinul 2, avand siruri de caractere (de maxim 30 caractere) pe post de chei (deci o pagina din arbore va contine intre 2 si 4 siruri de caractere):

// citeste o valoare intreaga N
// citeste N siruri de caractere si le insereaza pe rand in arbore (se poate porni de la functia de adaugare gata implementata - vezi sectiunea de resurse);
// afiseaza inaltimea arborelui
// afiseaza numarul de pagini
// Ex:
// input = 5 a b c d e
// output = 2 3
...
/*void afisare(Pagina *p, int nivel){
    ...
    {
        for(i = 1; i <= p->m; i++)
            printf("%s ", p->e[i].cheie);
        
        afisare(p->p0, nivel + 1);
...
void nr_pagini(Pagina *p, int nivel){
    int i;
    
    if(p != NULL)
    {
        pagini++;
        nr_pagini(p->p0, nivel + 1);
        
        for(i = 1; i <= p->m; i++)
            nr_pagini(p->e[i].p, nivel + 1);
    }
}
